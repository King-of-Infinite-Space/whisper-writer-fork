...
class Profile:
    ...
    def __init__(self, name: str, event_bus: EventBus):
        ...
        self.recording_mode = RecordingMode[
            self.config["recording_options"]["recording_mode"].upper()
        ]
        ...
        self.streaming_chunk_size = (
            self.transcription_manager.get_preferred_streaming_chunk_size()
        )
        self.result_handler = (
            StreamingResultHandler(self.output_manager) if self.is_streaming else None
        )
        self.current_session_id = None

        self.event_bus.subscribe(
            "raw_transcription_result", self.handle_raw_transcription
        )
        self.event_bus.subscribe(
            "transcription_finished", self.handle_transcription_finished
        )
    ...
    def start_transcription(self, session_id: str):
        ...
        self.event_bus.emit(
            ...
            f"({self.name}) {'Streaming' if self.is_streaming else 'Recording'}...",
        )
    ...
    def recording_stopped(self):
        ...
        if self.state == ProfileState.RECORDING:
            self.event_bus.emit(
                "profile_state_change", f"({self.name}) Transcribing..."
            )
    ...
    def should_stop_on_release(self) -> bool:
        ...
        return (
            self.state == ProfileState.RECORDING
            and self.recording_mode == RecordingMode.HOLD_TO_RECORD
        )
